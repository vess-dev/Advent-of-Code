	...
	def update(input_in, input_near):
		...
		for temp_seat in input_in[1]:
			...
			for temp_angle in seat_angles:
				if input_near == 4:
					if (temp_seat[0] + temp_angle[0], temp_seat[1] + temp_angle[1]) in input_in[2]:
						seat_close += 1
				elif input_near == 5:
					...
					while (0 <= seat_reach[0] < input_in[0]) and (0 <= seat_reach[1] < input_in[0]):
						...
						if seat_check in input_in[1]:
							if seat_check in input_in[2]:
								seat_close += 1
							break
			if seat_close == 0:
				if temp_seat not in input_in[2]:
					input_new[2].add(temp_seat)
			elif seat_close >= input_near:
				if temp_seat in input_in[2]:
					input_new[2].remove(temp_seat)
		return input_new
